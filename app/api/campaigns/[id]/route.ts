...
import { campaignFromRow, leadFromRow } from "@/lib/dashboard-data";
...
export async function GET(_req: NextRequest, { params }: Context) {
  ...
  return NextResponse.json({
    ...campaignFromRow(campaign),
    steps: steps.map((step, i) => ({ ...step, intervalDays: intervals[i] })),
    leads: (assignments ?? []).map((a) => ({ id: a.id, leadId: a.lead_id, assignedAt: a.assigned_at, lead: leadFromRow(a.leads as Record<string, unknown>), hasOverride: a.step_overrides != null })),
  });
}
