...
import { getDashboardCampaigns, getDashboardLeads, supabaseEnabled } from "@/lib/dashboard-data";

export default async function CampaignsPage() {
  const access = await getDashboardAccess();
  if (!access) redirect("/login");

  const [campaigns, leads] = supabaseEnabled() ? await Promise.all([
    getDashboardCampaigns(), getDashboardLeads(access),
  ]) : await Promise.all([
